...
- **Don't reach for a `self.fetch` monkeypatch via `evaluate_script` to intercept a worker's requests — it's structurally unreliable, and the reason is ordinary JS scoping, not a broken realm.** A module that captured a reference to `fetch` at import time keeps calling that reference; reassigning `self.fetch` afterward can't rebind it (closure semantics, true of a page too). `list_network_requests`/`get_network_request` observe the `Network` domain directly and need neither a monkeypatch nor visibility into module state.
