...
- **`extract_page`** — cleaned page HTML → OpenAI-compatible extraction endpoint → schema-conformant JSON. Params: `schema` (required, both the contract and the prompt), `selector` (subtree scoping, pierces open shadow roots), `source` (`dom` default; `served` is a not-implemented error, not a guess), `clean` (`standard`/`aggressive`/`none`), `maxChars`, `timeoutMs`, `model`, `baseUrl`, and `savePath` (write the JSON to a file; the response carries the path and usage counts only). Errors are distinguishable by name: `html_too_large`, `upstream_error` (status + body, key redacted), `timeout`. New env knobs `CDP_EXTRACT_BASE_URL` / `CDP_EXTRACT_MODEL` / `CDP_EXTRACT_API_KEY` / `CDP_EXTRACT_TIMEOUT_MS` / `CDP_EXTRACT_MAX_CHARS`.
- New profile group **`extraction`** (14 groups total): `CDP_TOOL_PROFILE` can now name `extraction`, and the default `full` listing grows to 49 entries (48 tools + `describe_tool`).
- **Unit tests `test/extract.test.ts` (22)** pinning schema validation (must be a `type:"object"` JSON Schema, `additionalProperties:true` refused, the property-description rule), every argument check (`selector`/`source`/`clean`/`maxChars`/`timeoutMs`/`model`/`baseUrl`), the `served` not-implemented error, `html_too_large`, and API-key redaction.
- New live harness **`test/extract-smoke.ts`** (`bun run extract:smoke`): a real page through the real loopback endpoint, proving the schema-conformant JSON round trip and the guard errors end to end.
- **Firefox wiring.** `extract_page` is backend-neutral by construction (its page work runs entirely through the driver's evaluate, no CDP-only capability), so it joins the Firefox registry: 35 of the 48 tools under `CDP_BROWSER=firefox`, 36 entries in the default listing.

## [2.3.0] - 2026-09-10

**Drive focus-gated pages on a background tab without stealing OS window focus.** Some sites gate a button behind `document.hasFocus()`/`visibilitychange` — the canonical case is Anthropic's Claude Code OAuth page (`claude.ai/oauth/*`), whose **Authorize** button stays disabled on a background tab, forcing an agent driving `claude -p` login to keep a human clicking. Two new Chrome-only tools port the mechanism of the [`danbuhler/claude-code-auto-authorize`](https://github.com/danbuhler/claude-code-auto-authorize) extension: `Emulation.setFocusEmulationEnabled` makes the page *believe* it is focused (hasFocus→true, visibility stays `visible`, focus events fire) while the real window stays put, and `Input.dispatchMouseEvent` delivers the **trusted** click (`isTrusted:true`) that a content script's synthesized events can never produce.

### Added

- **`focus_emulation { enabled }`** — the raw toggle: a tab reports focused/visible with no OS focus change. Call with `enabled:false` to restore.
- **`click_focus_gated { selector|text, timeoutMs, pollMs, keepFocus }`** — the one-call compose: enable focus emulation, poll until the button (CSS selector or exact visible-text, e.g. `"Authorize"`) exists **and** is enabled, trusted-click it, restore emulation (always, unless `keepFocus`, even on throw). Timeout error names the failure mode ("never located" vs "found but stayed disabled").
- New capability **`emulate.focus`** (Chrome declares it; BiDi's emulation module has no focus primitive, so both tools are absent from `tools/list` under `--browser firefox`, per ADR-001).
- New live harness **`test/focus-emulation-smoke.ts`** (`bun run focus:smoke`): a real headless Chrome serves a focus-gated Authorize fixture (disabled until `hasFocus()`), proves the gate is closed in a background tab (control), then `click_focus_gated` opens it and lands a trusted click — page log shows `gate:unlocked` + `CLICKED trusted=true` with the tab never activated. Plus unit tests `test/focus-emulation.test.ts` (16) pinning validation, capability gating, and manifest/docs/group registration.
